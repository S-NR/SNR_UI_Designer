...
void MX_LTDC_Init(void)
{
  ...
  hltdc.Init.Backcolor.Blue = 255;  /* Magenta background to distinguish from layer */
  ...
  /* Configure Layer 0 for RGB565 @ 480x272 starting at 0,0 */
  pLayerCfg.WindowX0 = 0;
  ...
  pLayerCfg.FBStartAdress = 0x24000000;  /* AXI SRAM D1 base address */
  ...
  /* Enable LTDC Layer 0 - CRITICAL for display output */
  // __HAL_LTDC_LAYER_ENABLE(&hltdc, 0);  /* DISABLED FOR TESTING */
  
  /* Explicitly disable Layer 1 to avoid interference */
  __HAL_LTDC_LAYER_DISABLE(&hltdc, 1);
  
  /* Enable the LTDC peripheral itself */
