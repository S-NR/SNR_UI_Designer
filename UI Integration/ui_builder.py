...
void DMA2D_FillRect(int x, int y, int w, int h, uint16_t color)
{
    ...
    /* Configure DMA2D for register-to-memory mode (fill) */
    extern DMA2D_HandleTypeDef hdma2d;
    
    /* Wait for any previous DMA2D operation to complete */
    while (hdma2d.Instance->CR & DMA2D_CR_START);
    
    /* Deinitialize to clear any previous configuration */
    HAL_DMA2D_DeInit(&hdma2d);
    
    /* Configure DMA2D for filling (R2M mode) */
    hdma2d.Instance = DMA2D;
    hdma2d.Init.Mode = DMA2D_R2M;  /* Register-to-memory (fill) */
    hdma2d.Init.ColorMode = DMA2D_OUTPUT_RGB565;  /* Match framebuffer format */
    hdma2d.Init.OutputOffset = LTDC_WIDTH - w;  /* Offset to next line */
    ...
    uint16_t *fb = g_pFramebuffer;
    for (int py = y; py < y + h; py++) {
        for (int px = x; px < x + w; px++) {
            fb[py * LTDC_WIDTH + px] = color;
        }
    }
}
